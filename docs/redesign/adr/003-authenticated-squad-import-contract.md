...
After explicit review, the encoded contract may be retained in `sessionStorage` for the current tab only. The authenticated planning endpoint parses the contract again, requires its entry ID to match the server session, verifies freshness for the target deadline, checks all players against current bootstrap data, and enforces squad composition, club limits, and starting-formation rules. Only then may imported picks represent the manager's private squad for scenario generation. Unlimited pre-season transfers are modeled without points hits. No durable storage is introduced by this phase.
...
Planning distinguishes the upcoming target Gameweek from the latest published squad. A valid confirmed import is authoritative before the target deadline because it represents private Pick Team changes; otherwise, the latest published squad is the baseline. The import expires two hours after the target deadline, allowing that Gameweek's official public picks to become authoritative. Row-level security blocks anonymous database access, while the application service role performs all reads and writes behind the existing manager session.
